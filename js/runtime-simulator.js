...
function parseInput(instructionList) {
    ...
    for (let i = 0; i < instructionList.length; i++) {
        ...
        if (format == undefined) {
            raiseError(`Line ${i + 1}: Instruction ${destructuredInstruction[0]} was not recognized.`);
            parsingResult.status = ParserStatus.ERR;
            parsingResult.errOnLines.push(i);
            break;
        }
        ...
        if (!(operands.length == (format === null || format === void 0 ? void 0 : format.length))) {
            raiseError(`Line ${i + 1}: ${operands.length} operands supplied but expected ${format === null || format === void 0 ? void 0 : format.length} operands.`);
            parsingResult.status = ParserStatus.ERR;
            parsingResult.errOnLines.push(i);
            break;
        }
...
function executeInstruction(destructuredInstruction) {
    ...
    switch (instructionType) {
        ...
        case I_TYPE:
            ...
            status = iTypeCallable(destructuredInstruction[1], // Destination register
        ...
        case R_TYPE:
            ...
            status = rTypeCallable(destructuredInstruction[1], // Destination register
        ...
        case U_TYPE:
            ...
            status = uTypeCallable(destructuredInstruction[1], // Destination register
